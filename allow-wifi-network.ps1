...
& scp.exe -o StrictHostKeyChecking=accept-new (Join-Path $root 'allow-wifi-network.sh') "$target`:$remote/allow-wifi-network.sh"
if ($LASTEXITCODE -ne 0) { throw 'Uploading the access helper failed.' }
& ssh.exe -tt -o StrictHostKeyChecking=accept-new $target "bash $remote/allow-wifi-network.sh"
