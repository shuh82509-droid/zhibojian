...
& scp.exe -o StrictHostKeyChecking=accept-new (Join-Path $root 'allow-detected-source.sh') "$target`:$remote/allow-detected-source.sh"
if ($LASTEXITCODE -ne 0) { throw 'Uploading the access helper failed.' }
& ssh.exe -tt -o StrictHostKeyChecking=accept-new $target "bash $remote/allow-detected-source.sh"
