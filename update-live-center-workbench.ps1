...
$bundleName = "live-center-workbench-update-$([DateTime]::UtcNow.ToString('yyyyMMddHHmmss')).tar.gz"
...
try {
  ...
  Write-Host '[2/3] Uploading the application update. Enter the SSH password.' -ForegroundColor Yellow
  Invoke-TransportRetry -FailureMessage 'Application update upload failed after 3 attempts.' -Action {
    & scp @sshOptions $bundlePath "${remote}:$incomingDir/$bundleName"
  }

  Write-Host '[3/3] Updating the existing Docker deployment. Enter SSH/sudo passwords when prompted.' -ForegroundColor Yellow
  $remoteCommand = "set -eu; cd '$runtimeDir'; test -s .env.coco; tar -tzf '$incomingDir/$bundleName' | grep -E '(^/|(^|/)[.][.](/|$))' && exit 1 || true; tar -xzf '$incomingDir/$bundleName' -C '$runtimeDir'; rm -f '$incomingDir/$bundleName'; chmod 700 deploy-remote.sh; ./deploy-remote.sh"
  & ssh @sshOptions -tt $remote $remoteCommand
  if ($LASTEXITCODE -ne 0) { throw 'Server update failed; copy the terminal output back to Codex.' }
} finally {
  Pop-Location
