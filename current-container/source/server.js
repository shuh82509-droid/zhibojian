...
import {
  ...
  parseEmploymentMessages, parseRecruitmentMessages, extractAnchorEvaluation, verifiedAnchorEvidence, feishuDocumentLink
...
async function feishuApi(req, res, url, routePath) {
  try {
    if (routePath === '/api/feishu/status') {
      ...
      return json(res, 200, {ok:true, appId, bot:'coco', configured:true, chats:Object.keys(feishuChats), documents:Object.keys(feishuDocs)});
    }
...
async function readLifecycleSnapshot(module) {
  if (module === 'anchors' && recoveryReadOnly && process.env.RECOVERY_ANCHOR_SNAPSHOT) {
