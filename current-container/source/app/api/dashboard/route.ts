...
async function dispatchStaffing(request: Request, date: string, roomName: string) {
  ...
  try {
    ...
    const response = await fetch(`https://app.fandow.top/fd-027340/dispatch-center/api/schedule?date=${encodeURIComponent(date)}`, {
      headers,
      cache: "no-store",
      redirect: "manual",
    ...
    const mapShift = (role: "anchor" | "assistant", value: unknown) => {
      const shift = Array.isArray(value) ? value : [];
      if (shift.length < 3 || !shift[2]) return null;
      return {
        role,
        name: String(shift[2]),
        startAt: `${date}T${String(shift[0] ?? "").padStart(5, "0")}:00+08:00`,
        endAt: `${date}T${String(shift[1] ?? "").padStart(5, "0")}:00+08:00`,
      };
    };
    const items = [
