...
async function feishuGet(path: string) {
  ...
  const response = await fetch(`${FEISHU_API}${path}`, { headers: { Authorization: `Bearer ${token}` } });
...
async function loadTargets(month: string): Promise<TargetMap> {
  ...
  const title = `${year}年${monthNumber}月目标汇总`;
  const sheet = sheets.find((item) => text(item.title).replace(/份/g, "") === title.replace(/份/g, ""));
  if (!sheet?.sheet_id) throw new Error(`KPI目标汇总缺少 ${title}`);
  const rows = await readValues(spreadsheet, `${sheet.sheet_id}!A1:O80`);
...
export async function loadBusinessOverview(date: string, forceRefresh = false): Promise<BusinessOverview> {
  ...
  const targets = targetResult.status === "fulfilled" ? targetResult.value : {};
  ...
  if (cumulativeOnlyRooms.length) warnings.push(`${cumulativeOnlyRooms.join("、")}源表未提供独立退款列，当前按源表累计GMV口径展示，未将缺失退款当作 0。`);
  const value: BusinessOverview = {
    ...
    targets: progress.map((item) => {
      ...
      return {
        ...item,
        target,
        completion: target && item.actual !== null ? Number((item.actual / target * 100).toFixed(1)) : null,
      };
    }),
    calendar: planning.calendar,
    concerns: planning.concerns,
    sources: {
      kpi: "https://jqx28l0j4lx.feishu.cn/wiki/Lb2vwOidyiViRBkGLIHcfTP5nWe?sheet=Xcrv3r",
      plan: "https://jqx28l0j4lx.feishu.cn/wiki/IFpowLFu1iiVzZk3kehc4QmUn8b",
    },
    warnings,
    fetchedAt: new Date().toISOString(),
  };
  cache = { key: date, value, expiresAt: Date.now() + CACHE_MS };
  return value;
}
