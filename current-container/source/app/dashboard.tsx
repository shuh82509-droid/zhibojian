...
const dateTime = (value: string) => value ? value.slice(0, 16).replace("T", " ") : "—";
const duration = (seconds: number | null) => seconds === null
  ? "待核验"
  : seconds ? `${Math.floor(seconds / 3600)}小时${Math.round(seconds % 3600 / 60)}分` : "0分";
const dashboardApiUrl = () => window.location.pathname.startsWith("/fd-027340/data-center/")
  ? "/fd-027340/data-center/api/dashboard"
  : "/api/dashboard";
const overviewApiUrl = () => window.location.pathname.startsWith("/fd-027340/data-center/")
  ? "/fd-027340/data-center/api/business-overview"
  : "/api/business-overview";
const calendarApiUrl = () => window.location.pathname.startsWith("/fd-027340/data-center/")
  ? "/fd-027340/live-center-workbench/api/calendar-overrides"
  : "/api/calendar-overrides";
const dispatchCenterUrl = "/fd-027340/live-center-workbench/#schedule";

export default function Dashboard({ initialView }: { initialView: "overview" | "data" }) {
  ...
  const loadDashboard = useCallback(async (date = "", session: string | null = null) => {
    ...
    try {
      if (pageView === "overview") {
        ...
        if (calendarResult.status === "fulfilled") {
          const calendarPayload = await calendarResult.value.json() as { ok?: boolean; items?: CalendarOverride[] };
          if (calendarResult.value.ok && calendarPayload.ok) setCalendarOverrides(calendarPayload.items ?? []);
        }
        return;
      }
  ...
  return <main className="app-shell">
    ...
    <section className="content">
      ...
      <section className="monthly-progress panel">
        ...
        <div className="target-grid">{overview?.targets.map((item) => <article key={item.code}><div><b>{item.room}</b><span>{item.daysFound} 个有效数据日 · {item.basis === "cumulative_gmv" ? "累计GMV" : "退后成交"}</span></div><strong>{item.completion === null ? "待核验" : `${item.completion}%`}</strong><div className="target-track"><i style={{ width: `${Math.min(100, item.completion ?? 0)}%` }} /></div><small>{money(item.actual)} / {item.target ? money(item.target) : "目标待核验"}</small><button className="monthly-trend-button" onClick={() => setMonthlyTrendCode((current) => current === item.code ? null : item.code)}>{monthlyTrendCode === item.code ? "收起每日趋势" : "查看每日趋势"}</button></article>) ?? <div className="overview-loading">正在计算本月成交…</div>}</div>
      ...
      <section className="calendar-panel panel">
        <div className="panel-heading"><div><p className="eyebrow">LIVE PLANNING CALENDAR</p><h2>直播日历与关注事项</h2><small>飞书月度规划为基线；机制或产品临时变化可在工作台手动覆盖并留痕</small></div><div className="calendar-actions"><button onClick={() => setCalendarEditor({ date: selectedDate, room: "官旗", title: "" })}>＋ 手动调整</button><a className="source-link" href={overview?.sources.plan} target="_blank" rel="noreferrer">查看规划原文 ↗</a></div></div>
        <div className="calendar-layout"><div className="month-calendar"><div className="calendar-weekdays">{["星期一","星期二","星期三","星期四","星期五","星期六","星期日"].map((day) => <b key={day}>{day}</b>)}</div><div className="calendar-days">{monthCalendar.map((cell, index) => <article className={cell.inMonth ? "" : "outside"} key={cell.date || `outside-${index}`} onClick={() => cell.inMonth && setCalendarEditor({ date: cell.date, room: "官旗", title: "" })}>{cell.inMonth ? <><time>{cell.day}</time><div>{cell.items.map((item) => <p className={`room-${item.room}`} key={`${item.room}-${item.title}`} onClick={(event) => { event.stopPropagation(); setCalendarEditor({ date: item.date, room: item.room, title: item.title }); }}><span>{item.room}</span>{item.title}<i>编辑</i></p>)}</div></> : null}</article>)}</div>{overview && !mergedCalendar.length ? <div className="empty-state compact">本月尚无直播事项，可点击日期手动新增</div> : null}</div><aside className="concern-list"><h3>各直播间本月规划</h3>{overview?.concerns.length ? <ul>{overview.concerns.map((item) => <li key={item}>{item}</li>)}</ul> : <p>本月日历表格上方暂未读取到直播间规划。</p>}{overview?.warnings.length ? <small>{overview.warnings.join("；")}</small> : null}</aside></div>
      ...
      <section className="panel visual-panel"><div className="panel-heading"><div><p className="eyebrow">LIVE ROOM VISUAL</p><h2>直播间视觉概览</h2></div><span className="badge">直播背景</span></div>{data?.visual && selected ? <div className="visual-content"><a className="visual-frame visual-link" href={selectedLiveRoomLink} target="_blank" rel="noreferrer" aria-label={`打开${selected.shop}直播间`}>{data.visual.background ? <img src={data.visual.background.url} alt={`${selected.shop}直播间背景`} /> : data.visual.avatarUrl ? <img src={data.visual.avatarUrl} alt={`${selected.shop}直播间头像`} /> : <span>{shopShortName[selected.shop]}</span>}<i><b>{selected.status === "live" ? "LIVE" : "REPLAY"}</b></i><strong>进入直播间 ↗</strong></a><div className="visual-copy"><p>{data.visual.background ? "当前直播间背景" : "当前直播主题"}</p><h3>{data.visual.title}</h3><div><span>直播间</span><b>{data.visual.accountName || selected.shop}</b></div><div><span>实时状态</span><b>{selected.status === "live" ? `直播中 · 在线 ${number(selected.online)}` : "该场已结束"}</b></div><a className="live-room-button" href={selectedLiveRoomLink} target="_blank" rel="noreferrer">打开抖音直播间 ↗</a><small>{data.visual.background ? `${data.visual.background.label}（${data.visual.background.updatedAt}）；未上传新背景时继续展示最近已确认版本。` : "该直播间尚未接入背景图，暂展示官方头像。"}</small></div></div> : <div className="empty-state compact">该场次暂无可用的直播间视觉资料</div>}</section>
      <section className="panel communication-panel"><div className="panel-heading"><div><p className="eyebrow">LIVE COMMUNICATION DRAFT</p><h2>目前沟通稿</h2></div><span className="badge">店铺专属</span></div><div className="communication-content"><span className="communication-icon">稿</span><div><p>当前直播间</p><h3>{selected?.shop ?? selectedShop}</h3><small>点击打开该直播间当前使用的沟通稿，可在飞书中查看与编辑。</small></div><a className="communication-link" href={selectedCommunicationDraftLink} target="_blank" rel="noreferrer" aria-label={`打开${selected?.shop ?? selectedShop}目前沟通稿`}>打开沟通稿 ↗</a></div></section>
      ...
      <section className="panel staffing-panel"><div className="panel-heading"><div><p className="eyebrow">LIVE STAFFING</p><h2>主播与助理排班</h2></div><span className="badge">{data?.staffingStatus?.source === "dispatch_center" ? "调度中心同步" : "营销排班"}</span></div>{data?.staffing.length && activeKey === trafficKey ? <><div className="staffing-list">{data.staffing.map((item, index) => <div className="staff-member" key={`${item.role}-${item.name}-${index}`}><span className={`staff-role ${item.role}`}>{item.role === "anchor" ? "主播" : "助理"}</span><div><b>{item.name}</b><small>班次 {dateTime(item.startAt).slice(11)} – {dateTime(item.endAt).slice(11)}</small></div><i className="live-dot" /></div>)}</div><div className="staffing-source"><span>{data.staffingStatus?.reason}</span><a href={dispatchCenterUrl} target="_top">打开调度中心 ↗</a></div></> : <div className="empty-state compact"><b>当前班次待同步</b><span>{data?.staffingStatus?.reason ?? "当前没有匹配到该直播间的排班"}</span><a className="detail-button" href={dispatchCenterUrl} target="_top">去调度中心核对</a></div>}</section>
